...
export function build_plugins(options: BuildOptions): Configuration["plugins"] {
  ...
  const html_plugins = (HTML_PAGES ?? []).map(
    (page) =>
  ...
  const copy_patterns = [
    // { from: "source", to: "dest" },
    {
      from: path.resolve(options.paths.src, "assets/*.pdf"),
      to: path.resolve(options.paths.output, "assets/[name][ext]"),
    },
    {
          ...
          patterns: [
            {
              ...
              to: path.resolve(options.paths.output, `${HTML_PATH}/partials/[name][ext]`),
            },
          ],
        })
      );
