...
import type { BuildPaths, BuildMode } from "./webpack_config/types/types";
...
const ENTRY_VALUES: Record<string, string> = Object.fromEntries(
  Object.entries(ENTRY_PATHS).map(([key, value]) => [key, path.resolve(__dirname, DEFAULT_PATHS.src, value)])
);
...
export default (env: EnvVariables) => {
  const paths: BuildPaths = {
    ...
    favicon: path.resolve(__dirname, DEFAULT_PATHS.src, "assets/images/favicon.ico"),
  };
